...
public class CycleActivity {
    ...
    @Inject public CycleActivity(ICycleService cycleService) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "returns cycle corresponding to Id",
            response = GetCycleResponse.class
    )
    @ApiResponses(value = {
    ...
    @Path("/{cycleId}")
    public Response get(@PathParam("cycleId") String cycleId) {
        log.info("[get] received get request for cycle id: " + cycleId);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "returns all cycles lying between skip and take, when ordered by Id.",
    ...
    public Response get(@PathParam("take") Integer take, @PathParam("skip") Integer skip) {
        log.info("[get] received get request for all cycles, take: " + take + ", skip: " + skip);
        return cycleService.getCycles(take, skip);
    }
}
